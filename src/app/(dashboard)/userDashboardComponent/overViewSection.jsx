...
import { useGetReadingList, useAllSalesReport } from "../../../lib/getData";
...
export default function OverViewSection() {
  ...
  const { data: readingList, isLoading: isReadingLoading } =
    useGetReadingList(userEmail);

  // ─── 📊 DYNAMIC DATA CALCULATIONS ───

  
  const readingListData = readingList?.message || readingList || [];
  const booksReadCount = readingListData.length;

  // ২. Pending Deliveries (সেলস লেজার থেকে নির্দিষ্ট ইউজারের পেন্ডিং ট্র্যাকিং)
  const salesLedgerArray = PendingData?.salesLedger || [];
  const pendingSalesForCurrentUser = salesLedgerArray.filter(
    (item) => item.userEmail === userEmail && item.status === "pending",
  );

  // অল্টারনেটিভ পেন্ডিং ডেলিভারি কাউন্ট (যদি salesReportData-তে থাকে)
  const salesReportData =
    PendingData?.salesReport || PendingData?.message || [];
  const userSales = salesReportData.filter(
    (item) => item?.userEmail === userEmail,
  );
  const pendingDeliveriesCount = userSales.filter(
    (item) =>
      item?.status?.toLowerCase() === "pending" ||
      item?.deliveryStatus?.toLowerCase() === "pending",
  ).length;

  // ফাইনাল পেন্ডিং কাউন্ট (দুটির মধ্যে যেটি ব্যবহার করতে চান)
  const finalPendingCount =
    pendingDeliveriesCount > 0
      ? pendingDeliveriesCount
      : pendingSalesForCurrentUser.length;

  // ৩. Total Spent (ইউজারের সফলভাবে কেনা বা ধার নেওয়া বইয়ের মোট ফি/খরচ)
  const totalSpentAmount = pendingSalesForCurrentUser.reduce(
    (sum, item) => sum + (item.amount || 0),
    0,
  );

  // ৩. কারেন্সি ফরম্যাট করার জন্য (যেমন: $20.00)
  const formattedTotalSpent = new Intl.NumberFormat("en-US", {
    style: "currency",
    currency: "USD", // আপনার ডাটার কারেন্সি অনুযায়ী
  }).format(totalSpentAmount);

  // Loading State
  if (isSalesLoading || isReadingLoading) {
    return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white border border-[#EFECE6] rounded-2xl p-6 shadow-sm flex items-center gap-5 transition-all duration-200 hover:shadow-md">
            ...
            <div>
              ...
              {/* ✅ FIX: formattedTotalSpent ব্যবহার করা হয়েছে, কারণ এতে কারেন্সি আইকন আগে থেকেই যুক্ত থাকে */}
